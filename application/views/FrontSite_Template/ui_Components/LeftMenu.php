...
if(LoggedIn()){
    ...
    foreach($GetForms as $key => $row){
        ...
        $LeftNav =explode("/", $row['FormPath']);
        //list($NavBar) = $LeftNav[0];

        $arrayLeftMenuList[$key] = array();
        $arrayLeftMenuList[$key]['NavBarMenuLink'] = $LeftNav[0];
        $arrayLeftMenuList[$key]['LeftMainMenu'] = $LeftNav[1];
        if(isset($LeftNav[2])){
            $arrayLeftMenuList[$key]['LeftSubMenu'] = $LeftNav[2];
        }
...
<?php
   ...
   // print_r($arrayLeftMenuList);exit();
    ...
    foreach($arrayLeftMenuList as $key => $menu_items){
        ...
        if($arrayLeftMenuList[$key]['HaveSubMenus'] != '0'){
